<template>
  <div v-if="isLoading === true">
    ...
    <div class="bg-base-100 shadow-xl  p-4 my-3 ">
      <!-- 部员 -->
      <h2 class=" font-bold text-lg mb-4 block">桑梓的{{ filter_res[0].position }}们</h2>
      <div class="flex flex-wrap">
        <div v-for="(item ,index) in filter_res" :key="index">
          <div class=" w-[100px] h-auto block text-center  shadow-xl m-2 py-2 rounded-md  brightness-95 hover:-translate-y-1.5 transition-all	duration-400	cursor-pointer " @click="pushToUser(item)" :class=" { 'bg-sky-300':  item.gender === 1,  'bg-pink-200': item.gender === 0 }">
            ...
            <div class="">
              ...
              <div class="block badge badge-ghost my-1 mx-auto">{{ item.position }}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class=" bg-base-100 shadow-xl  p-4 my-3" v-if="items[0].session === selectedYear || display_all">
        ...
        <div class="flex flex-wrap">
          <div v-for="(item ,index) in items" :key="index">
            <!-- 部员 -->
            <div  v-if="item.session " class=" w-[100px] h-auto block text-center  shadow-xl m-2 py-2 rounded-md  brightness-95 hover:-translate-y-1.5 transition-all	duration-400	cursor-pointer " @click="pushToUser(item)" :class=" { 'bg-sky-300':  item.gender === 1,  'bg-pink-200': item.gender === 0 }">
              <div class="avatar">
                <div class="w-20 rounded-full">
                  <img src="@/assets/images/默认头像.svg" />
                </div>
...
let filter_res = ref<User[]>([]);
const isLoading = ref<Boolean>(false);
onMounted(async () => {
  const res = await userService.getDefaultUserDataApi();
  if (res.status === 200) {
    console.log(res.data);
    filter_res.value = res.data.filter((item) => item.position === "老师");
    processed_res.value = sortData(res.data);
    console.log("filter_res", filter_res.value);
